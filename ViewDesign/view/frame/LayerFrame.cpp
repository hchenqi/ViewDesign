...
Size _LayerFrame_Base::OnSizeRefUpdate(Size size_ref) {
	...
	if (size != child_size) { size = child_size; layer.DestroyFramebuffer(); }
	return size;
}

void _LayerFrame_Base::OnChildSizeUpdate(ViewBase& child, Size child_size) {
	if (size != child_size) { size = child_size; layer.DestroyFramebuffer(); SizeUpdated(size); }
}
...
void _LayerFrame_Base::OnDraw(Canvas& canvas, Rect draw_region) {
	...
	if (Scale current_scale = canvas.GetTransform().GetScale(); scale != current_scale) { scale = current_scale; layer.DestroyFramebuffer(); }
