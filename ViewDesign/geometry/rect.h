...
struct Rect {
	...
	constexpr bool Contains(const Point& point) const { return point >= TopLeft() && point < BottomRight(); }
	constexpr bool Contains(const Rect& other) const { return other.TopLeft() >= TopLeft() && other.BottomRight() <= BottomRight(); }

	constexpr Rect Union(const Rect& other) const {
		if (IsEmpty()) { return other; } if (other.IsEmpty()) { return *this; }
		Point pl1 = TopLeft(), ph1 = BottomRight(), pl2 = other.TopLeft(), ph2 = other.BottomRight();
